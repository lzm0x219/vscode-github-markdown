...
After an extension host crash or an unclean exit, saved originals remain available. To restore them when sync is disabled, enable `githubMarkdown.mermaid.syncTheme` and then disable it in the same workspace; values you changed yourself remain unchanged. Mermaid settings are shared across windows, so enabling sync takes ownership of the shared settings without assuming that an earlier window has closed.
